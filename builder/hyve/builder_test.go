...
func XTestBuilder_ImplementsBuilder(t *testing.T) {
	var raw interface{}
	raw = &Builder{}
...
func XTestBuilderPrepare_Defaults(t *testing.T) {
	var b Builder
	config := testConfig()
...
func XTestBuilderPrepare_BootWait(t *testing.T) {
	var b Builder
	config := testConfig()
...
func XTestBuilderPrepare_DiskSize(t *testing.T) {
	var b Builder
	config := testConfig()
...
func XTestBuilderPrepare_HTTPPort(t *testing.T) {
	var b Builder
	config := testConfig()
...
func XTestBuilderPrepare_HyveArgs(t *testing.T) {
	var b Builder
	config := testConfig()

	// Test with empty
	delete(config, "hyveargs")
	warns, err := b.Prepare(config)
	if len(warns) > 0 {
		t.Fatalf("bad: %#v", warns)
	}
	...
	if !reflect.DeepEqual(b.config.HyveArgs, [][]string{}) {
		t.Fatalf("bad: %#v", b.config.HyveArgs)
	}

	// Test with a good one
	config["hyveargs"] = [][]interface{}{
	...
	if !reflect.DeepEqual(b.config.HyveArgs, expected) {
		t.Fatalf("bad: %#v", b.config.HyveArgs)
	}
}
